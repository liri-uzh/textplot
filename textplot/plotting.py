...
def plot_graph(
    ...
    layout_kwargs["seed"] = layout_kwargs.get("seed", 42)  # Set a default seed for reproducibility
    ...
    try:
        ...
        raise Exception(
            f" Check that the layout_kwargs are compatible with the chosen algorithm (see the documentation:" \
            f" https://networkx.org/documentation/stable/reference/generated/networkx.drawing.layout.spring_layout.html)."
    ...
    if verbose:
        print(f"Layout calculated using {layout_algorithm.__name__} with parameters: {layout_kwargs}")
    ...
    if subgraph_nodes:
        ...
        node_color_map = {node: colors[i % num_subgraph_nodes] for i, node in enumerate(subgraph_nodes)}
        ...
        subgraph_edge_color = colors[0] if colors else main_graph_color  # Use first color or default
        subgraph_node_color = colors[0] if colors else main_graph_color  # Use first color or default
        ...
        if label_subgraph_nodes:
            nx.draw_networkx_labels(G, pos, labels={node: node for node in subgraph_nodes}, font_size=font_size, font_weight="bold", alpha=1.0)
...
def set_args():
    ...
    parser.add_argument("input_file", type=str, help="Input GML file") # "data/outputs/examples/8set_ALL.name_text_source_ASCII_cleaned_w_sentiment_t200_skim5_bw20k.gml"
    parser.add_argument("--output_file", type=str, default=None, help="Output file name. If not provided, will be inferred from input file") # "data/outputs/examples/8set_ALL.name_text_source_ASCII_cleaned_w_sentiment_t200_skim5_bw20k.png"
    
    parser.add_argument("--subgraph_nodes", type=str, nargs="*", help="List of subgraph nodes")
    
    # layout arguments
    parser.add_argument("--layout_algorithm", type=str, default="spring_layout", 
        choices=[
            "spring_layout", 
            "forceatlas2_layout",
    ...
    parser.add_argument("--max_iter", type=int, default=200, help="Maximum iterations for layout algorithm")
    parser.add_argument("--iterations", type=int, default=200, dest="max_iter", help="Maximum iterations for layout algorithm (alias for max_iter)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for layout algorithm")
    parser.add_argument("--k", type=float, default=0.1, help="Spring layout parameter k")

    # plotting arguments
    parser.add_argument("--palette", type=str, default="blend:#7AB,#EDA", help="Color palette for subgraph nodes. See seaborn palettes.")
    parser.add_argument("--label_subgraph_nodes", action="store_true", help="Label subgraph nodes")
    parser.add_argument("--font_size", type=int, default=12, help="Font size for labels")
    parser.add_argument("--node_size", type=int, default=10, help="Node size")
    parser.add_argument("--negative_alpha", type=float, default=0.5, help="Alpha value for non-subgraph nodes")
    parser.add_argument("--figsize", type=tuple_type, default=(12, 12), help="Figure size")
    parser.add_argument("--main_graph_color", type=str, default="grey", help="Color for main graph nodes and edges")
